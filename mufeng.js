...
const program = new commander.Command(packageJson.name)
    ...
    .arguments('<mufengblog action>')
    .usage(`${chalk.green('<mufengblog action>')}`)
    .action(name => {
        action = name
    })
    ...
    .allowUnknownOption()
    .on('--help', () => {
        console.log(`${chalk.cyan('mufengblog')} ${chalk.green('deploy --user <github user> --path <vuepress docs path> --remote <github repository address>')}`)
        console.log()
        console.log(`${chalk.cyan('mufengblog')} ${chalk.green('post --path <article path>')}`)
    })
    .parse(process.argv)

if(typeof action === 'undefined') {
    console.log(`${chalk.cyan('mufengblog')} ${chalk.green('your action')}`)
    console.log()
    console.log('For example:')
    console.log(`${chalk.cyan('mufengblog')} ${chalk.green('deploy <user>')}`)
    console.log()
    console.log(
        `Run ${chalk.cyan(`${'mufengblog'} --help`)} to see all options.`
    )
    process.exit(1)
}
